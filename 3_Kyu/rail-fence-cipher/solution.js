...
function encodeRailFenceCipher(string, numberRails) {
    ...
    return rails.map((rail) => rail.filter((char) => char != '__________').join('')).join('');
}
...
function decodeRailFenceCipher(string, numberRails) {
    ...
    let rails = Array.from({ length: numberRails }, (_, i) =>
        Array.from({ length: string.length }, (_, j) =>
            !i ? (j % spacesBetween == 0 ? stringArray.shift() : '__________') : '__________'
        )
    );
    ...
    return rails[0].reduce(
        (acc, _, i) =>
            (acc += rails.map((rail) => (rail[i] != '__________' ? rail[i] : '')).join('')),
        ''
    );
}
