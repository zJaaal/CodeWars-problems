...
const andJoinWhere = (array1, array2, conditions) => {
    for (let i = 0; i < array1.length; i++) {
        if (!(conditions[0]([array1[i], array2[i]]) && conditions[1]([array1[i], array2[i]]))) {
            array1.splice(i, 1);
            array2.splice(i, 1);
        }
    }
...
const createDataStructure = (
    ...
    structure.forEach((group) => {
        createDataStructure(group, keys, data, currentDepth, depth, groups, '', group[0], callback);
    });
};
...
var query = function () {
    ...
    this.execute = function () {
        ...
        if (myQuery.where.and.length == 1 && myQuery.from.length == 2) {
            ...
            let [array1, array2] = orJoinWhere(myQuery.from[0], myQuery.from[1], myQuery.where.and);
